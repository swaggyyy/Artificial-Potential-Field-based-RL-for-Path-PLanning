...
class myenv(Env):
    ...
    #
    #     ## TODO 计算与目标点间的力 ？？？？？？？？？？？？？？？？？？？？？？？？？？？
    #     # 合力与距离正相关，距离越近力越小（源于人工势场的设计）因此取倒数作为奖励函数
    #     F_att_reward = 1 / (self.Eta_att * self.dists[len(self.Pobs)])
    #
    #     # 障碍物斥力奖励函数设计
    #     for j in range(len(self.Pobs)):
    #         if self.dists[j] >= self.d0:
    #             self.F_rep_ob[j] = np.array([0, 0])
    #         else:
    #             F_req_obs_abs = self.Eta_rep_ob * (1 / self.dists[j] - 1 / self.d0) / (self.dists[j] ** 2)
    #             F_req_obs = F_req_obs_abs * self.unite_vec[j]
    #             self.F_rep_ob[j] = F_req_obs
    #
    #     F_req = np.sum(self.F_rep_ob)
    #
    #     F_req_reward = -np.linalg.norm(F_req) * 0.1
    #
    #     reward_collision = 0
    #
    #     done1 = False
    ...
    #             break
    #
    #     if ((self.Pi[0] - self.Pg[0]) ** 2 + (self.Pi[1] - self.Pg[1]) ** 2) ** 0.5 < 1:
    #         reward_goal = 100
    #         done2 = True
    #     else:
    #         reward_goal = -1
    ...
    #         done3 = False
    #
    #     if self.d < self.Pi[1] or -self.d > self.Pi[1]:
    #         reward_edg = -100
    #         done4 = True
    #     else:
    #         reward_edg = 0
    ...
    #     reward = F_att_reward + F_req_reward + reward_collision + reward_goal + reward_count + reward_edg
    #
    #     self.current_state = {
